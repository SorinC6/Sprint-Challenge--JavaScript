...
console.log(`${tyrannosaurus.name} weight: ${tyrannosaurus.weight}`);

// What was the diet of a velociraptor?
console.log(`${velociraptor.name} is ${velociraptor.diet}`);

// How long was a stegosaurus?
console.log(`${stegosaurus.name} is ${stegosaurus.length} long`);

// What time period did tyrannosaurus live in?
console.log(`${tyrannosaurus.name} lived in ${tyrannosaurus.period} period`);
...
//for this exercis I'll be using the university sorted array from first exercises
const uni = [];
for (let i = 0; i < universities.length; i++) {
	if (universities[i].includes('Uni')) uni.push(universities[i]);
}

//alternative without using university array
// for(let i=0;i<graduates.length;i++){
...
const lowerCase = zooAnimals.map((item) => {
	//let lowerCaseName=item.animal_name.toLowerCase();
	return `Name: ${item.animal_name.toLowerCase()}`;
});
...
const lessThen = (nr) => {
	return zooAnimals.filter((item) => {
		return item.population < nr;
	});
};
largerPopulation = lessThen(5);
